    ...
    return executeIngestionRun(
      ...
      async () => {
        ...
        if (newStreams.length === 0) {
          return {
            result: { created: 0, skipped: 0, total: rawStreams.length },
            summary: {
              recordsScanned: rawStreams.length,
              recordsSkipped: rawStreams.length,
            },
          };
        }
        ...
        return {
          result: { ...result, total: rawStreams.length },
          summary: {
            recordsScanned: rawStreams.length,
